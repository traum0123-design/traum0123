(function(){
  ...
  async function loadAll(){
    // Reset state
    qs('nextCursor').value = '';
    const p0 = paramsFromFilters(); // to validate filters
    let firstPass = true;
    while(true){
  ...
  function init(){
    const q = qs('btnQuery'); if(q){ q.addEventListener('click', ()=>{ qs('nextCursor').value=''; load(false); }); }
